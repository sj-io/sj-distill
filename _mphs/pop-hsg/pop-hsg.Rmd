...
```{r}
cTN <- list(157, 167, 047)
cAR <- "Crittenden"
cMS <- list("DeSoto", "Tunica", "Tate", "Marshall")
```
...
hsgAR <- get_acs(
  ...
  mutate(county = 1, .before = "GEOID")

hsgST <- rbind(hsgTN, hsgAR, hsgMS)
```
...
vacST <- hsgST %>% 
  ...
  pivot_wider(names_from = variable,
              values_from = estimate) %>% 
  mutate(vacancy = (B25002_003 / B25002_001)*100) %>% 
  select(-c(4:5))
```
...
```
```{r}
tractAR <- tracts("AR", cAR)
tractTN <- tracts("TN")
tractMS <- tracts("MS")

tracts <- rbind(tractAR, tractTN, tractMS) %>% 
  select(-NAME)
```

```{r}
vacGeo <- tracts %>% 
  right_join(vacST, by = "GEOID") %>% 
  select(GEOID, vacancy)
```

```{r}
tm_shape(vacGeo) +
  tm_polygons(col = "vacancy")
```

...
```{r}
acsg <- read_sf("../_data/geo/acsg.shp")
shelby_blocks <- read_sf("../_data/geo/blocks.shp")
```

```{r}
mv15 <- acsg %>% 
  filter(year == 2015) %>% 
  mutate(estimate = estimate * 1.09257667)

## inflation adjusted per https://www.census.gov/programs-surveys/acs/guidance/comparing-acs-data/2020/5-year-comparison.html
...
```{r}
mv_shift <- mv20 %>% 
  left_join(st_drop_geometry(mv_interpolate),
            by = "GEOID",
            suffix = c("_2020", "_2015")) %>% 
  mutate(mv_shift = estimate_2020 - estimate_2015,
         mv_log = case_when(
           mv_shift > 0 ~ log(mv_shift),
           mv_shift < 0 ~ log(abs(mv_shift)) * -1,
           TRUE ~ mv_shift
         ))
```
...
```{r}
mv20 <- mv20 %>% 
  mutate(mv_log = case_when(
           estimate > 0 ~ log(estimate),
           estimate < 0 ~ log(abs(estimate)) * -1,
           TRUE ~ estimate
         ))
```
...
mapview(mv20, 
        zcol = c("estimate", "mv_log"), 
        layer.name = c("Median Value", "Median Value (log)"), 
        label = ld(mv20$estimate),
        col.regions = brewer.pal(11, "RdYlGn"),
        legend = c(TRUE, FALSE),
        homebutton = c(TRUE, FALSE),
        hide = c(TRUE, FALSE))
```

### Map: Change in Median Value, 2015-2020

```{r}
mapview(mv_shift, 
        zcol = c("mv_shift", "mv_log"),
        layer.name = c("Change in Median Value", "Change in Median Value (Log)"),
        label = ld(mv_shift$mv_shift),
        col.regions=brewer.pal(11, "RdYlGn"),
        legend = c(TRUE, FALSE),
        homebutton = c(TRUE, FALSE),
        hide = c(TRUE, FALSE)) 
...
# Tenure {data-orientation=rows data-navmenu="Tenure"}
...
# Income {data-navmenu="Tenure"}

## B25118: Tenure by Household Income in the Past 12 Months

```{r}
v118 <- read_csv("../_data/vB25118.csv")
```
...
# Tenure {data-navmenu="Geographical Mobility"}
...
# Median Income {data-navmenu="Geographical Mobility"}
...
# Poverty Level {data-navmenu="Geographical Mobility"}

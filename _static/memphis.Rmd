...
ggplot() +
   shelby_basemap +
   geom_sf(data = mv_shift,
           aes(fill = mv_shift),
           color = NA, alpha = 0.8) +
   scale_fill_distiller(palette = "PuOr", direction = -1) + 
   labs(fill = "Shift, 2011-2015 to\n2016-2020 ACS",
        title = "Change in median value",
        subtitle = "Shelby County, Tennessee") + 
   theme_void()
```

```{r}
up <- mv_shift %>% filter(mv_shift > 0) %>% 
  mutate(mv_log = log(mv_shift))
down <- mv_shift %>% filter(mv_shift < 0) %>% 
  mutate(mv_log = log(abs(mv_shift)) * -1)
mv_log <- mv_shift %>% 
  mutate(mv_log = log(mv_shift))
```

```{r}
mapview(up, zcol = "mv_log", col.regions=brewer.pal(9, "Greens")) +
mapview(down, zcol = "mv_log", col.regions=rev(brewer.pal(9, "Reds")))
```
